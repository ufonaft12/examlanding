...
build.js           sass compile + critical CSS + share-card metadata
og.png             1200×630 share card — a render of the page itself
...
- **`config.json` is the single source of truth** — brand, both variant headlines, social proof, prizes, teams, odds, CTA labels, product names, share-card metadata and every fineprint string. Verified rather than asserted: replacing every value in the config with a traceable marker and re-rendering the page in four states leaves only interface furniture ("Stake", "Potential Payout", "VS"), the 1 / X / 2 market notation, the regulatory 18+ line and computed sums. Even the goals ladder is config — the slider bounds, step and tick labels are derived from the keys of `oddsByLine`, so re-pricing the market in config moves the control with it instead of leaving the markup lying about where it stops. There is deliberately **no bundled copy of the config in JS**: if the fetch fails the page shows an "Offers unavailable / Try again" state rather than silently serving stale duplicated data.
...
- **The share card is built, not hydrated.** Title, description, canonical, Open Graph and the Twitter card are generated from `config.json` by `build.js` and baked into `<head>`. They cannot be set at runtime like the rest of the copy: the crawlers behind Facebook, Twitter/X, Slack, WhatsApp and Google do not execute the page's JavaScript, so a title assigned in `script.js` is a title they never see. Baking them at build time keeps config the single source anyway. `og.png` is a 1200×630 render of the actual page rather than a mocked-up graphic — regenerate it with `chrome --headless --window-size=1200,630 --screenshot=og.png <url>` after a design change. It costs the page nothing: only crawlers ever fetch it.
