...
index.html         document + generated critical CSS      11.5 KB  (3.9 KB gz)
style.css          generated — deferred stylesheet        15.6 KB  (3.9 KB gz)
script.js          LandingApp: state, A/B, interactions   16.8 KB  (5.6 KB gz)
config.json        all copy, offers, odds, teams, A/B      2.3 KB  (1.1 KB gz)
                                        shipped: ~46 KB raw · ~15 KB gzipped
```
...
- **The reward is scrolled to, not left below the fold.** On a phone both reveal panels open under the viewport: the player acts, and the prize and its CTA are somewhere off screen. Both flows now scroll the panel in — bottom-aligned, because that is where the CTA sits — and only when it is genuinely out of view, so nothing jumps on desktop. The easing is hand-rolled on `requestAnimationFrame`: `scrollIntoView({behavior: 'smooth'})` is a silent no-op in some engines and WebViews, and this is the one moment in the funnel where the CTA has to actually arrive. Geometry comes from `offsetTop`/`offsetHeight` rather than `getBoundingClientRect`, because the panels animate in with `scaleY` and a rect measured mid-animation aims the scroll at the wrong place. A timer guarantees the final position if rAF is starved, revealing a panel resizes the document so the target is re-checked once motion ends, and any real user scroll cancels it outright.
